...
const Home = () => {
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 animate-fade-in">
        ...
        {filteredInstructions.length > 0 ? (
          ...
          <div className="text-center py-12 text-gray-500 bg-white rounded-2xl border border-gray-100">
            <p className="text-lg">Nu am găsit nicio instrucțiune care să se potrivească.</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="flex flex-col justify-start items-center w-full h-[calc(100dvh-80px)] overflow-hidden bg-[#F5F4EF] px-4">
      <div className="flex flex-col w-full max-w-5xl gap-8 md:gap-16 mt-8 md:mt-16 h-auto">
        ...
        <div className="w-full text-center animate-slide-up">
          <h1 className="text-2xl md:text-4xl font-extrabold text-[#0A2B1C] mb-4 tracking-tight">Cum te putem ajuta?</h1>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 animate-slide-up" style={{animationDelay: '0.1s'}}>
          <CategoryCard 
            title="Kiosk" 
            icon={<Tablet className="w-8 h-8 md:w-12 md:h-12 text-[#1D5337] transition-colors" strokeWidth={1.5} />}
            count={instructions.filter(i=>i.category==='Kiosk').length}
            loading={loading}
          />
          <CategoryCard 
            title="Casă" 
            icon={<Monitor className="w-8 h-8 md:w-12 md:h-12 text-[#1D5337] transition-colors" strokeWidth={1.5} />}
          ...
          <CategoryCard 
            ...
            icon={
              <div className="flex gap-1">
                <Monitor className="w-6 h-6 md:w-8 md:h-8 text-[#1D5337] transition-colors" strokeWidth={1.5} />
                <Monitor className="w-6 h-6 md:w-8 md:h-8 text-[#1D5337] transition-colors" strokeWidth={1.5} />
              </div>
            }
